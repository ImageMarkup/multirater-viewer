...
export function generateRaterAgreements(tiles, allRaterData) {
    ...
    $.each(spxMarkupMap, function(raterCount, spxList) {
        /*I am cutting off a consensus at more than 3.. which is technically 4*/
        if (raterCount < 4) {
            addRaterOverlay(
                tiles,
                spxList,
                heatMapColors[raterCount],
                "multiRater raterClass" + " raterCount" + raterCount + " moreThan" + raterCount
            );
        }
    ...
    for (var i = 1; i <= 3; i++) {
        console.log("Analyzing raterClass moreThan" + i)
        var spxConsensus = []
        $.each(spxMarkupMap, function(raterCount, spxId) {
            if (raterCount > i) {
                spxConsensus.push(spxId)
            }


        })
        console.log(spxConsensus, i)

        var mrc = 'moreThan' + i; //#multiRaterClassName
        var mrcId = $$("raterInfoDataTable").find(function(obj) { return obj.raterClassName == mrc }, true);
        console.log(mrcId)

        $$("raterInfoDataTable").updateItem(mrc.id, {

            spxMarkedForCurrentFeature: spxConsensus,
            raterTotalFeaturesSeen: "",
            spxMarkedForCurrentFeatureCount: spxConsensus.length
        });
